...
[2026-03-15] BRAND_ID_NOT_TENANT_ID — V4.1: All new tables and queries must use `brand_id`, not `tenant_id`. Existing code still uses `tenant_id` until migration M001 completes. After migration, grep entire codebase for remaining `tenant_id` references.
[2026-03-15] SUPABASE_JS_PRIMARY — V4.1: supabase-js (PostgREST) is the primary query client. No new Prisma Client query code. Prisma is ONLY for `prisma migrate`. Check `docs/REFERENCE.md` for API tiers.
[2026-03-15] URL_IS_SCOPE_SOURCE — V4.1: Brand/branch scope must come from URL params (`/b/[brandId]/br/[branchId]/`), NEVER from localStorage, React Context, or Zustand. Use `useScope()` hook.
