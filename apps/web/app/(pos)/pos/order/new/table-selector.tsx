        ...
        <div key={zoneName}>
          ...
          <div className="grid grid-cols-3 gap-3 sm:grid-cols-4 md:grid-cols-5 lg:grid-cols-6">
            {zoneTables.map((table) => {
              const isSelectable =
                selectable &&
                (table.status === "available" || (allowOccupied && table.status === "occupied"));
              ...
              return (
                <button
                  ...
                  className={cn(
                    ...
                    statusColors[table.status] ?? "border-gray-300 bg-gray-50",
                    isSelected && "ring-primary ring-2 ring-offset-2",
                    isSelectable && "cursor-pointer active:scale-95",
                    !isSelectable && "cursor-default opacity-70"
                  )}
                >
                  <span className="text-lg font-bold">Bàn {table.number}</span>
                  <span className="text-xs">{getTableStatusLabel(table.status)}</span>
                  {table.capacity && (
                    <span className="mt-0.5 text-xs opacity-60">{table.capacity} chỗ</span>
                  )}
                </button>
              );
            })}
