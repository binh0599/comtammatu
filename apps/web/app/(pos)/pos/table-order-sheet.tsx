...
import { createOrder, confirmOrder, updateOrderStatus, addOrderItems } from "./orders/actions";
import {
  Badge,
  Button,
  Sheet,
    ...
    <div className="flex flex-1 flex-col items-center justify-center p-6">
      <div className="w-full max-w-xs space-y-6">
        <div className="text-center">
          ...
          <p className="text-muted-foreground mt-1 text-sm">Chọn số khách cho đơn hàng mới</p>
      ...
      <div className="space-y-2 p-4">
        ...
        {totalGuests > 0 && (
          <div className="flex items-center gap-2 rounded-lg border bg-muted/30 px-3 py-2 text-sm">
            ...
            <span>
              {totalGuests}/{tableCapacity} khách
            </span>
            {remainingSeats > 0 && (
        ...
        {orders.map((order, index) => {
          ...
          return (
            ...
            >
              ...
              <div className="mt-1 font-semibold">{formatPrice(order.total)}</div>
            </button>
          );
        })}
      </div>
...
function OrderViewMode({
  ...
  const subLabel =
    totalOrders && totalOrders > 1 && orderIndex && tableNumber
  ...
  return (
    <div className="flex flex-1 flex-col">
        ...
        <div className="rounded-lg border p-4">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3">
              <span className="text-muted-foreground text-sm">{order.item_count} món</span>
              {order.guest_count && order.guest_count > 0 && (
      ...
      <div className="mt-auto space-y-2 border-t p-4">
        ...
        {canAddItems && (
          <Button variant="outline" onClick={onAddItems} className="w-full gap-2">
            <PlusCircle className="size-4" aria-hidden="true" />
            Thêm món
          </Button>
        )}
        ...
        {order.status === "draft" && (
          <Button onClick={handleConfirm} disabled={isPending} className="w-full gap-2">
        ...
        {showBackToList && onBackToList && (
          <Button variant="ghost" className="w-full gap-2" onClick={onBackToList}>
...
export function TableOrderSheet({
  ...
  const [internalMode, setInternalMode] = useState<
    "view" | "create" | "add-items" | "list" | "guest-count"
  >(initialMode);
  const [selectedOrderFromList, setSelectedOrderFromList] = useState<ActiveOrder | null>(
    activeOrder
  );
  ...
  const viewedOrder = currentMode === "view" ? (selectedOrderFromList ?? activeOrder) : null;
  ...
  const tableNumber = tableLabel.startsWith("Bàn ")
    ? parseInt(tableLabel.replace("Bàn ", ""), 10)
    : undefined;
  ...
  return (
    <Sheet open={open} onOpenChange={handleOpenChange}>
      <SheetContent side="bottom" className="flex h-[85vh] flex-col rounded-t-2xl" showCloseButton>
        <SheetHeader>
