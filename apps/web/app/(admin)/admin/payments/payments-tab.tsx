...
export function PaymentsTab({ payments, branches }: PaymentsTabProps) {
  ...
  const filtered = useMemo(() => {
    return payments.filter((p) => {
      ...
      if (branchFilter !== "all" && p.pos_terminals?.branches?.id !== Number(branchFilter))
        return false;
      if (searchQuery) {
  ...
  return (
    <div className="space-y-4">
      ...
      <div>
        ...
        <p className="text-muted-foreground">Xem và quản lý các giao dịch thanh toán</p>
      </div>
      ...
      <div className="flex flex-wrap items-center gap-3">
        ...
        <div className="text-muted-foreground ml-auto text-sm">
          ...
          <span className="font-medium text-black">{formatPrice(totalAmount)}</span>
          {totalTip > 0 && (
            <>
              {" "}
              &middot; Tip: <span className="font-medium text-black">{formatPrice(totalTip)}</span>
            </>
          )}
        </div>
      </div>
            ...
            {filtered.length === 0 ? (
              ...
              filtered.map((p) => (
                <TableRow key={p.id}>
                  ...
                  <TableCell>{p.pos_terminals?.branches?.name ?? "—"}</TableCell>
                  ...
                  <TableCell className="font-mono text-sm">{p.reference_no ?? "—"}</TableCell>
                  <TableCell className="text-right">
                      ...
                      {p.status === "completed" && (
                        <AlertDialog>
                          <AlertDialogTrigger asChild>
                            <Button variant="ghost" size="icon" title="Hoàn tiền">
                              <RotateCcw className="h-4 w-4" />
                            </Button>
                          </AlertDialogTrigger>
                          <AlertDialogContent>
                            <AlertDialogHeader>
                              <AlertDialogTitle>Hoàn tiền giao dịch</AlertDialogTitle>
                              <AlertDialogDescription>
                                Bạn có chắc muốn hoàn tiền {formatPrice(Number(p.amount))} cho đơn{" "}
                                {p.orders?.order_number ?? `#${p.order_id}`}? Hành động này không
                                thể hoàn tác.
                              </AlertDialogDescription>
                            </AlertDialogHeader>
                            <AlertDialogFooter>
      ...
      >
        <DialogContent className="max-w-lg">
          <DialogHeader>
            ...
            <DialogDescription>Thông tin giao dịch #{detailPayment?.id}</DialogDescription>
          </DialogHeader>
          {detailPayment && (
            <div className="grid gap-3 py-4 text-sm">
              <div className="grid grid-cols-2 gap-2">
                ...
                <span className="font-medium">
                  {detailPayment.orders?.order_number ?? `#${detailPayment.order_id}`}
                </span>
              </div>
              <div className="grid grid-cols-2 gap-2">
                <span className="text-muted-foreground">Chi nhánh:</span>
                <span>{detailPayment.pos_terminals?.branches?.name ?? "—"}</span>
              </div>
              <div className="grid grid-cols-2 gap-2">
              ...
              <div className="grid grid-cols-2 gap-2">
                ...
                <span className="font-medium">{formatPrice(Number(detailPayment.amount))}</span>
              </div>
              ...
              <div className="grid grid-cols-2 gap-2">
                ...
                <span>
                  {detailPayment.orders ? formatPrice(Number(detailPayment.orders.total)) : "—"}
                </span>
              </div>
              <div className="grid grid-cols-2 gap-2">
                <span className="text-muted-foreground">Trạng thái:</span>
                <Badge className={STATUS_BADGE[detailPayment.status] ?? ""}>
              ...
              {detailPayment.reference_no && (
                <div className="grid grid-cols-2 gap-2">
                  ...
                  <span className="font-mono">{detailPayment.reference_no}</span>
                </div>
              )}
              <div className="grid grid-cols-2 gap-2">
