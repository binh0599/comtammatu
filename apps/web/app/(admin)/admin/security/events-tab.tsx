...
function SeverityBadge({ severity }: { severity: string }) {
  switch (severity) {
    ...
    case "critical":
      return <Badge variant="destructive">{getSeverityLabel(severity)}</Badge>;
    default:
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid gap-4 md:grid-cols-3">
        <Card>
          <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
            <CardTitle className="text-sm font-medium">Đăng nhập thất bại (24h)</CardTitle>
        ...
        <Card>
          <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
            <CardTitle className="text-sm font-medium">Cảnh báo (24h)</CardTitle>
        ...
        <Card>
          <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
            <CardTitle className="text-sm font-medium">Nghiêm trọng (24h)</CardTitle>
      ...
      <div className="flex items-center gap-3">
        ...
        <span className="text-muted-foreground text-sm">{filteredEvents.length} sự kiện</span>
      </div>
            ...
            {filteredEvents.length === 0 ? (
              <TableRow>
                <TableCell colSpan={7} className="text-muted-foreground h-24 text-center">
                  Chưa có sự kiện bảo mật nào. Sự kiện sẽ được ghi nhận khi có hoạt động đáng chú ý.
                </TableCell>
              </TableRow>
            ) : (
                  ...
                  <TableCell>
                    {event.details ? (
                      <div>
                        ...
                        >
                          {expandedId === event.id ? "Thu gọn" : truncateJson(event.details)}
                        </Button>
                        {expandedId === event.id && (
