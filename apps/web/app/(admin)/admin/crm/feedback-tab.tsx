              ...
              {feedback.length === 0 ? (
                ...
                feedback.map((item) => (
                  <TableRow key={item.id}>
                    ...
                    <TableCell>{item.orders?.order_number ?? "-"}</TableCell>
                    <TableCell>{item.branches?.name ?? "-"}</TableCell>
                    <TableCell>
                      <RatingDisplay rating={item.rating} />
                    </TableCell>
                    <TableCell className="max-w-[200px] truncate">{item.comment ?? "-"}</TableCell>
                    <TableCell>
                      {item.response ? (
      ...
      >
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              Đánh giá từ {respondingItem?.customers?.full_name ?? "Khách vãng lai"}
          ...
          {respondingItem && (
            <div className="space-y-4">
              ...
              <div className="space-y-2 rounded-md border p-3">
                ...
                {respondingItem.comment && <p className="text-sm">{respondingItem.comment}</p>}
              </div>

              {/* Existing response display */}
              {respondingItem.response && (
                <div className="space-y-2 rounded-md border border-green-200 bg-green-50 p-3">
                  <p className="text-sm font-medium text-green-800">Phản hồi của nhà hàng:</p>
                  <p className="text-sm text-green-700">{respondingItem.response}</p>
                </div>
              )}
